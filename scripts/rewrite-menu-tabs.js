...
function tabsFor(menu) {
  ...
  if (menu.top === "대시보드") {
    ...
    if (hasAny(text, ["보고서"])) return ["보고서 목록", "생성·예약", "다운로드 이력", "감사 대응"];
  ...
  if (menu.top === "회원·고객확인") {
    ...
    if (menu.code === "ADM-MEM-008") {
      return ["심사 큐", "신청·증빙", "OCR·안면인증", "검토·보완", "승인·반려", "제한·후속조치", "감사로그"];
    }
    if (hasAny(text, ["KYC", "고객확인", "재이행", "CARF", "본인 인증", "OTP"])) {
  ...
  if (menu.top === "거래·마켓 관리") {
    ...
    if (hasAny(text, ["보유수량"])) return ["보유 요약", "회원·자산별 상세", "스냅샷·대사", "변동 이력"];
    if (hasAny(text, ["거래량", "거래 현황"])) return ["거래 요약", "실시간 거래 모니터링", "기간별·페어 분석", "회원 영향도"];
    return ["거래 요약", "주문·체결 상세", "이상거래 징후", "처리·정정 이력", "감사로그"];
  }

  if (menu.top === "입출금·지갑") {
    if (menu.left1 === "입출금 정책" || hasAny(text, ["한도", "설정", "코인별 입출금 상태"])) return ["정책 요약", "대상별 설정", "점검·중단·재개", "예외·승인", "변경 이력"];
    if (menu.left1 === "지갑 관리" || hasAny(text, ["주소 관리", "핫/콜드", "자산 이전"])) return ["지갑 요약", "잔고·상태 모니터링", "이전·운영 작업", "위험·알림", "작업 이력"];
    if (menu.left1 === "자산 대사" || hasAny(text, ["대사", "분리보관", "고객자산", "회사자산"])) return ["대사 요약", "원장·온체인 비교", "차이·부족액", "보정·보고", "감사 이력"];
    if (hasAny(text, ["오입금", "반환"])) return ["접수 요약", "증빙·검토", "반환 승인", "처리 결과", "감사로그"];
  ...
  if (menu.top === "정산·회계") {
    if (hasAny(text, ["지급", "회수", "리워드", "프로모션", "추천인"])) return ["대상 요약", "지급·회수 요청", "승인·실행", "실패·보정", "정산 이력"];
    if (hasAny(text, ["마감"])) return ["마감 요약", "검증·차이", "조정·승인", "확정·보고", "재마감 이력"];
  ...
  if (menu.top === "고객지원·서비스") {
    ...
    if (hasAny(text, ["앱 버전", "업데이트", "점검"])) return ["운영 요약", "대상·조건 설정", "배포·예약", "예외·롤백", "변경 이력"];
